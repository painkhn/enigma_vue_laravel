...
<template>
    <div class="relative w-full h-auto">
        ...
        <ul class="main__categories-list flex flex-col gap-2 text-gray-800 dark:text-white mb-5">
            <li v-for="category in $page.props.categories" :key="category.id" class="mb-4 flex justify-between">
                <a :href="route('filter_by_category', category.id)" class="transition-all hover:text-red-400 font-medium p-2 dark:bg-zinc-600 bg-gray-100 rounded-md hover:border-l-2 hover:border-red-400">{{ category.name }}</a>
                <button @click="deleteVisibleChange(category)">
