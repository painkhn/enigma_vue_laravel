    ...
    <div class="w-full bg-white border-b border-slate-200">
        <header class="header max-w-7xl w-full h-14 grid grid-cols-3 items-center mx-auto my-0 gap-2 rounded-md">
            ...
            <nav class="justify-self-center justify-center flex items-center gap-3 w-full">
                <Link :href="'/'" class="transition-all flex items-center h-14 px-2 hover:text-red-400 font-medium hover:border-b-2 hover:bg-slate-50 hover:border-red-400">
                    Главная
                </Link>
                <Link :href="route('themeCreate')" class="transition-all flex items-center hover:text-red-400 h-14 font-medium hover:border-b-2 hover:bg-slate-50 px-2 hover:border-red-400">
                    Создать тему
                </Link>
                <Link :href="'/'" v-if="$page.props.auth.user && $page.props.auth.user.is_admin == 1" class="header__admin-link transition-all flex px-2 items-center hover:text-red-400 hover:bg-slate-50 h-14 font-medium hover:border-b-2 hover:border-red-400">
            ...
            <Link
                ...
                class="rounded-md px-3 font-semibold py-2 text-black ring-1 ring-transparent transition hover:text-red-400 focus:outline-none focus-visible:ring-[#FF2D20] dark:text-white dark:hover:text-white/80 dark:focus-visible:ring-white justify-self-end"
            >
